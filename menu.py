...
class Mainwindow(ui2.Ui_Unframewindow, QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        # self.setui(self)
        self.setupUi(self)
        self.setWindowTitle('网络流量对抗样本生成与检测系统')
        ...
        self.tableWidget_2.setColumnCount(7)  # 设置表格为七列
        # 创建表头
        self.tableWidget.setHorizontalHeaderLabels(['srcip', 'dstip', 'sport', 'dport', 'proto', 'data'])
        self.tableWidget_2.setHorizontalHeaderLabels(['srcip', 'dstip', 'sport', 'dport', 'proto', 'data','class'])
        # 禁止编辑
    ...
    def getshowdata(self,):
        ...
        for i in range(length, len(data_csv_label)):
            ...
            data_csv_label.loc[k, 'label'] = data_csv_label.loc[i, 'label']
            # data_csv_label['classes'][k] = data_csv_label['classes'][i]
            data_csv_label.loc[k, 'classes'] = data_csv_label.loc[i, 'classes']
            # data_csv_label.loc['label'][k] = data_csv_label.loc['label'][i]
            # data_csv_label.loc['classes'][k] = data_csv_label.loc['classes'][i]
            k+= 1
        j = 1
        i = 0
        count = 0
        while i < len(data_csv_data):
            if data_csv_data['flowmember'][i] == j:
                ...
                data_csv_data.loc[i, 'label'] = data_csv_label.loc[j - 1, 'label']
                data_csv_data.loc[i, 'classes'] = data_csv_label.loc[j - 1, 'classes']
                i += 1
            else:
                j = j + 1
                #         print("i",i)
    ...
    def curtail_pcap(self):
        ...
        QMessageBox.information(self, '生成pcap', '生成成功！')
    ...
    def startdetection(self):
        ...
        data_orginal['classes']='adversarial'
    ...
    def saveshowdata(self,data_csv_data):
        ...
        worksheet.write(0,2,label='classes')
        ...
        for i in range(len(data_csv_data)):
            ...
            worksheet.write(i + 1, 2, label=str(data_csv_data['classes'][i]))
        ...
        for i in range(len(data_orginal)):
            for j in range(len(data_show)):
                ...
                if data_orginal['data'][i][2:-1] == data_show['data'][j][2:-1]:
                    ...
                    # data_orginal.loc[i, 'label'] = data_show.loc[j, 'label']
                    data_orginal.loc[i, 'classes'] = data_show.loc[j, 'classes']
                    # print(data_orginal['classes'][i],data_show['classes'][j])
        return data_orginal
    ...
    def savelabelshown(self, data_orginal):
        ...
        worksheet.write(0, 6, label='classes')
        ...
        for i in range(len(data_orginal)):
            ...
            worksheet.write(i+1,6, label=str(data_orginal['classes'][i]))
